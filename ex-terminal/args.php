...
function parse_commands($stdin){
	...
	while ($i < $stdin_len){
		...
		if ($stdin[$i] == ';'){ 								// if we hit a command delimiter
			...
			array_push($commands, array(						// push the command
				...
				"args"	=> $args,
				//"plain" => substr($stdin, 0, $i-2) // mw might need this later (the plain text of the original command)
			)); 	
	...
	array_push($commands, array(					// push the command
		"cmd"	=> array_shift($args), 
		"args" 	=> $args, 
		//"plain"	=> substr($stdin, 0, $i) // mw might need this later (the plain text of the original command)
	)); 	
	return $commands;
}
